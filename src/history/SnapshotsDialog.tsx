...
export function SnapshotsDialog(props: SnapshotsDialogProps) {
  ...
  return <div className="modal-backdrop"><section className="modal wide-modal" role="dialog" aria-modal="true" aria-labelledby="changes-title">
    <div className="modal-header"><div><small>REVERSIBLE EDITING</small><h2 id="changes-title">Snapshots & What Changed?</h2></div><button aria-label="Close snapshots" onClick={props.onClose}>×</button></div>
    <div className="snapshot-layout">
      <aside className="snapshot-sidebar">
        ...
        {props.snapshots.map((snapshot) => <div className={reference === snapshot.id ? 'snapshot-item active' : 'snapshot-item'} key={snapshot.id}>
          ...
          <div><button onClick={() => props.onRestore(snapshot.id)}>Restore</button><button onClick={() => props.onRemove(snapshot.id)}>×</button></div>
        </div>)}
      </aside>
      <div className="diff-panel">
        <div className="diff-header"><strong>{changes.length} structural changes</strong><span>Reference → current project</span></div>
        <div className="diff-list">{changes.length === 0 ? <div className="validation-ok">✓ No structural changes from this reference.</div> : changes.slice(0, 500).map((change, index) => <article className={`diff-entry ${change.kind}`} key={`${change.path}-${index}`}>
